...
class MyBrush : public UIInterface {
 ...
 private:
  ...
  //accessing mask vector
  void setVal(int row, int col, float value);//set the element of array onto the mask vector
  float getVal(int row, int col);//get the element of array from the mask vector
  //mask distributions
  void constDist(int radius);
  void linDist(int radius);
  void quadDist(int radius);
  void gausDist(int radius);
  void specDist(int radius);
  //line stuff
  void drawThinLine1(Color col, int x0, int y0, int x1, int y1);//slope >= -1 and <=1
  void drawThinLine2(Color col, int x0, int y0, int x1, int y1);//slope the rest
  void fillLine(Color col, int x00, int y00, int x01, int y01, int x10, int y10, int x11, int y11);
  //circle stuff
  void circle(int radius,int thickness);
  //extra stuff
  int round(float d);
};
